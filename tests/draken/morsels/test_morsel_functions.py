...
def test_copy_with_filters_applies_mask_and_column_selection():
    ...
    table = pa.table(
        {"a": [0, 1, 2, 3], "b": ["p", "q", "r", "s"], "c": [True, False, True, False]}
    )
    morsel = draken.Morsel.from_arrow(table)
...
def test_method_chaining():
    ...
    result = morsel.select(["a", "b"]).take([0, 2]).rename(["first", "second"])
...
def test_unicode_column_values():
    ...
    table = pa.table({"col": ["hello", "世界", "مرحبا", "🎉", "Привет"]})
    morsel = draken.Morsel.from_arrow(table)
...
def test_complex_method_chaining():
    ...
    result = morsel.select(["a", "c", "d"]).take([0, 2, 4]).rename(["alpha", "charlie", "delta"])
...
def test_take_then_select_then_rename():
    ...
    table = pa.table({"x": [1, 2, 3], "y": [4, 5, 6], "z": [7, 8, 9]})
    morsel = draken.Morsel.from_arrow(table)
...
def test_copy_with_both_filters_applied_correctly():
    ...
    table = pa.table({"a": [1, 2, 3, 4], "b": [10, 20, 30, 40], "c": [100, 200, 300, 400]})
    morsel = draken.Morsel.from_arrow(table)
