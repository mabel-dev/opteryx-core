...
def view_in_catalog(monkeypatch):
    ...
    def resolve(relation, telemetry, catalog_cache=None, memo=None, *, plan_context):
        if relation == "heavy_planets":
            return "view", views_module._view_plan_from_definition(VIEW, plan_context=plan_context)
        return real_resolve(relation, telemetry, catalog_cache, memo=memo, plan_context=plan_context)
