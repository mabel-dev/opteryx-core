...
"""
StringArenaBuilder — produces DrakenStringArena from appended values.
...
cdef class StringArenaBuilder:
    ...
    def __cinit__(self, Py_ssize_t length, Py_ssize_t arena_capacity,
        ...
        self._arena = alloc_string_arena(DRAKEN_STRING, <size_t>length, <size_t>arena_capacity)
        self._length = length
        self._next_index = 0
    ...
    cdef DrakenStringArena* finish(self) except NULL:
        if self._finished:
            raise RuntimeError("StringArenaBuilder.finish called twice")
        ...
        self._finished = True
        cdef DrakenStringArena* out = self._arena
    ...
    def finish_handle(self):
        cdef DrakenStringArena* a = self.finish()
        return StringArenaHandle._wrap(a)
    ...
    cdef inline void _append_with_ptr(self, Py_ssize_t index, const char* src, Py_ssize_t length) except *:
        ...
        cdef DrakenStringSlot* slot = &self._slots[index]
        cdef Py_ssize_t arena_off

        if length <= STR_INLINE_MAX:
            str_init_inline(slot, <const uint8_t*>src, <uint32_t>length)
        else:
            ...
            str_init_extern(slot, <const uint8_t*>src, <uint32_t>length, <uint64_t>arena_off)
    ...
    cdef inline void _set_null(self, Py_ssize_t index) except *:
        ...
        str_init_null(&self._slots[index])
        self._next_index += 1
...
cdef class StringArenaHandle:
    """Owning wrapper around a finished DrakenStringArena. Test-only."""
    ...
    cdef StringArenaHandle _wrap(DrakenStringArena* arena):
        cdef StringArenaHandle h = StringArenaHandle.__new__(StringArenaHandle)
        h._arena = arena
        h._owns = True
        return h
    ...
    cdef inline DrakenStringSlot* _slot(self, Py_ssize_t i) except NULL:
    ...
    cpdef bint is_inline(self, Py_ssize_t i):
        cdef DrakenStringSlot* s = self._slot(i)
        return str_length(s) <= STR_INLINE_MAX

    cpdef Py_ssize_t slot_length(self, Py_ssize_t i):
        return <Py_ssize_t>str_length(self._slot(i))

    cpdef bytes slot_bytes(self, Py_ssize_t i):
        cdef DrakenStringSlot* s = self._slot(i)
        cdef Py_ssize_t n = <Py_ssize_t>str_length(s)
        cdef const uint8_t* p = str_data(s, self._arena.arena)
        return p[:n]
    ...
    cpdef bint slots_equal(self, Py_ssize_t i, Py_ssize_t j):
        cdef DrakenStringSlot* a = self._slot(i)
        cdef DrakenStringSlot* b = self._slot(j)
        return str_equals(a, self._arena.arena, b, self._arena.arena) != 0
