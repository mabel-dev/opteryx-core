...
def test_evaluate_and_append_morsel_literal_emits_constant_vector():
    ...
    assert result.column(b"k").__class__.__name__ == "ConstantVector"
    assert result.column(b"k").to_pylist() == [42, 42, 42]
    assert result.column(b"x").to_pylist() == [1, 2, 3]


def test_projection_node_keeps_constant_vector_on_morsel_literal_projection():
    morsel = Morsel.from_vectors(["x"], [vector_from_sequence([1, 2, 3])])
...
def test_evaluate_and_append_morsel_mixed_non_literal_and_literal_keeps_constant():
    morsel = Morsel.from_vectors(["x"], [vector_from_sequence([1, 2, 3])])
...
def test_projection_node_mixed_non_literal_and_literal_keeps_constant():
    morsel = Morsel.from_vectors(["x"], [vector_from_sequence([1, 2, 3])])
...
def test_projection_node_arrow_input_literal_uses_morsel_constant_path():
...
def test_projection_node_arrow_input_mixed_preserves_constant_literal():
    table = pa.table({"x": pa.array([1, 2, 3], type=pa.int64())})
