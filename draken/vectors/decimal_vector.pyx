...
cdef DecimalVector from_arrow(object array):
    ...
    vec._unified_view = draken_vector_from_dense(
        vec.ptr.data, <uint32_t>vec.ptr.length, DRAKEN_INT64, vec.ptr.null_bitmap)
    return vec
...
cpdef DecimalVector from_int64_vector(Integer64Vector source, int precision, int scale):
    ...
    if src_uv.data_length == 1:
        ...
        out._unified_view = draken_vector_from_constant(
            out.ptr.data, <uint32_t>n, DRAKEN_INT64,
            &_CONST_NULL_BYTE if src_uv.validity != NULL else NULL)
        return out
