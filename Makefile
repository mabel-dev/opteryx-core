...
help: ## Show this help message
	...
	@# [a-zA-Z0-9_-]: target names with DIGITS (tpch-sf1, tpch-sf100, tpcds-001,
	@# m4-sweep) were silently absent from this listing until the character class
	@# admitted digits — the help text claimed they did not exist.
	@awk 'BEGIN {FS = ":.*?## "} /^[a-zA-Z0-9_-]+:.*?## / {printf "  \033[1;33m%-18s\033[0m %s\n", $$1, $$2}' $(MAKEFILE_LIST)
...
# in docs/ and the M4 design notes was quoted at, and repointing it silently
# would rewrite the meaning of all of them.
#
# skene-only: TPC-H is measured against the skene mirror exclusively — the
# plain-parquet variant (the old `tpch` body) was removed 2026-08-16.
#
# Every scale passes the same four gates before a query runs:
#
#  1. PARQUET SOURCE. testdata/tpch_<sf> is not vendored (SF100 alone is ~39GB)
#     and has no in-tree generator: every scale here was produced out-of-band by
#     tpchgen-cli (github.com/clflushopt/tpchgen-rs, `cargo install tpchgen-cli`)
#     with `--parts 16`. The 16-numbered-files-per-table layout (nation/region
#     on part .1 only) is tpchgen-cli's convention, not ours, so the gate
#     reproduces it with the same tool and flags rather than a from-scratch
#     writer that would lay the files out differently. The parquet is an
#     intermediate; the skene mirror below is the benchmark artifact.
#
#  2. SKENE MIRROR, STAMPED ON THE LAYOUT, not just on the directory. A mirror
#     written before row groups were packed into files — or at format v1 — is a
...
#     clickbench-skene and job. Dropping it rebuilds an UNCOMPRESSED mirror into
#     the same path and nothing downstream would say so — the suite would report
#     a different number for a dataset nobody knew had changed. Deployed data
#     takes the OPPOSITE posture; see the clickbench-skene comment for why.
#
#  4. DUCKDB BASELINE. duckdb/results.sf<sf>.json is what the runner compares
#     against, for time AND for result shape — the shape check is the only
#     correctness gate this suite has. Running without a baseline silently
#     downgrades the benchmark to "some numbers", so a missing one is a hard
#     stop naming the target that makes it, never a warning.
#
# The baselines are regenerated by the matching tpch-sf<sf>-duckdb targets, kept
# separate because calibration is slow and machine-dependent.

define tpch_suite
	@test -d testdata/tpch_$(1) || command -v tpchgen-cli >/dev/null 2>&1 || { \
		echo "ERROR: testdata/tpch_$(1) not found and tpchgen-cli is not on PATH"; \
		echo "       install it with:  cargo install tpchgen-cli"; \
		exit 1; \
	}
	@test -d testdata/tpch_$(1) || { \
	...
	@test -s tests/performance/tpch/duckdb/results.sf$(1).json || { \
		...
		echo "       the suite compares timings AND result shapes against it — generate it with:"; \
		echo "         make tpch-sf$(1)-duckdb"; \
		exit 1; \
	}
	@clear || true
...
tpch: tpch-sf10 ## Alias for tpch-sf10 (the scale docs/ and the design notes quote)

tpch-sf1-duckdb: ## Re-run the DuckDB SF1 calibration (regenerates duckdb/results.sf1.json)
	@$(PYTHON) tests/performance/tpch/duckdb/runner.py --scale 1

tpch-sf10-duckdb: ## Re-run the DuckDB SF10 calibration (regenerates duckdb/results.sf10.json)
	@$(PYTHON) tests/performance/tpch/duckdb/runner.py --scale 10

tpch-sf100-duckdb: ## Re-run the DuckDB SF100 calibration (regenerates duckdb/results.sf100.json)
	@$(PYTHON) tests/performance/tpch/duckdb/runner.py --scale 100

.PHONY: tpch tpch-sf1 tpch-sf10 tpch-sf100 tpch-sf1-duckdb tpch-sf10-duckdb tpch-sf100-duckdb
...
# the two datasets a quoted coverage number came from. The old spellings stay
# as aliases.
#
# COVERAGE, NOT PERFORMANCE (see runner.py's docstring): this runs each of the
# 99 queries once and reports pass/fail/timeout, so there is no DuckDB timing
# baseline here and no shape comparison — unlike the TPC-H targets.
#
# Gates, in order:
#
#  1. PARQUET SOURCE, generated via DuckDB's vendored dsdgen
...
#     VERSION, not the directory: a v1 tree, or one packed before row groups
#     were grouped into files, is a different set of objects under different
#     names and the converter refuses to write over it — so a missing stamp
#     with a populated directory means "regenerate", which is what the rm does.
#     The parquet source stays reachable for comparison via
#     `python tests/performance/tpcds/runner.py --scale 1` (no --variant).
#
#  3. CODEC. `lz4` is NOT optional — WriteOptions::for_fast_reads, the local
#     posture every mirror in this tree uses (architect 2026-08-11). See the
#     clickbench-skene comment for why deployed data takes the opposite one.
...
tpcds: tpcds-sf1 ## Alias for tpcds-sf1

tpcds-001: tpcds-sf001 ## Alias for tpcds-sf001
...
tpch-bench-duckdb: tpch-sf10-duckdb ## Alias for tpch-sf10-duckdb (the runner's default scale)
