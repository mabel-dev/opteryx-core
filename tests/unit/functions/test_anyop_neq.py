...
def _test_neq_comparison(literal, test_value, expected_result, _type=pa.string()):
    ...
    assert result == [expected_result], (
        f"Expected {literal} != {test_value} to be {expected_result}, got {result[0]}"
    )
...
def test_comparison_with_string_lists():
    ...
    assert result == [1], (
        "Expected 'apple' != ['apple','banana','cherry'] to be True (as 'banana','cherry' are different)"
    )

    array = vector_from_arrow(pa.array([["apple", "apple", "apple"]], type=pa.list_(pa.string())))
    result = vector_anyop_neq("apple", array).to_pylist()
    assert result == [0], (
        "Expected 'apple' != ['apple','apple','apple'] to be False (as all elements are 'apple')"
    )
    ...
    result = vector_anyop_neq("apple", array).to_pylist()
    assert result == [1], (
        "Expected 'apple' != ['Apple','apple','APPLE'] to be True (case sensitivity)"
    )
    ...
    assert result == [1], (
        "Expected 'a' != ['a','aa','aaa'] to be True (as 'aa','aaa' are different)"
    )
...
def test_comparison_with_binary_lists():
    ...
    assert result == [1], (
        "Expected b'data' != [b'data',b'info',b'bytes'] to be True (as others are different)"
    )

    array = vector_from_arrow(pa.array([[b"data", b"data", b"data"]], type=pa.list_(pa.binary())))
    result = vector_anyop_neq(b"data", array).to_pylist()
    assert result == [0], (
        "Expected b'data' != [b'data',b'data',b'data'] to be False (as all elements are b'data')"
    )
    ...
    array = vector_from_arrow(
        pa.array([[b"\x00\x01\x02", b"\x00\x01\x03"]], type=pa.list_(pa.binary()))
    )
    ...
    array = vector_from_arrow(
        pa.array([[large_binary, large_binary + b"y"]], type=pa.list_(pa.binary()))
    )
    ...
    array = vector_from_arrow(
        pa.array([[b"\xf0\x9f\x98\x80", b"\xf0\x9f\x98\x81"]], type=pa.list_(pa.binary()))
    )  # 😀, 😁 emojis
...
def test_comparison_with_nulls():
    ...
    result = vector_anyop_neq("a", array).to_pylist()
    assert result == [0], (
        "Expected 'a' != [None,'a',None] to be False (null skipped, 'a' equals literal)"
    )
    _test_neq_comparison(-1, -2, 1, pa.int64())  # -1 != -2 -> True
    _test_neq_comparison(0, -1, 1, pa.int64())  # 0 != -1 -> True
...
def test_comparison_binary_edge_cases():
    ...
    _test_neq_comparison(
        b"\xf0\x9f\x98\x80", b"\xf0\x9f\x98\x81", 1, pa.binary()
    )  # different emoji bytes
...
def test_comparison_list_edge_cases():
    ...
    result = vector_anyop_neq("a", array).to_pylist()
    assert result == [0], (
        "Expected 'a' != [None,None,None] to be False (all nulls skipped, no non-null differs from 'a')"
    )
...
def test_comparison_list_dates():
    ...
    assert result[0] == 1, (
        f"Expected date comparison to return True for different dates, got {result}"
    )
