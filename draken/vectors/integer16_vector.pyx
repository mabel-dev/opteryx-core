...
from libc.stdint cimport int16_t, int32_t, int64_t, uint8_t, uint64_t, intptr_t
...
cdef class Integer16Vector(Vector):
    ...
    def from_constant(cls, value, length, is_null=False):
        ...
        vec._unified_view.itemsize = 2
        vec._unified_view.type = DRAKEN_INT16
    ...
    def __cinit__(self, size_t length=0, bint wrap=False):
        ...
        self._unified_view.itemsize = 2
        self._unified_view.type = DRAKEN_INT16
        if not wrap:
