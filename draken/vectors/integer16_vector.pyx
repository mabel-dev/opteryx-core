...
from libc.stdint cimport int16_t, int32_t, int64_t, uint8_t, uint32_t, uint64_t, intptr_t
...
cdef class Integer16Vector(Vector):
    """Fixed-width signed 16-bit integer vector (INT16, itemsize=2)."""
    ...
    def from_constant(cls, value, length, is_null=False):
        ...
        cdef Integer16Vector vec = Integer16Vector(<size_t>length)
        ...
        vec._unified_view = draken_vector_from_constant(
            vec.ptr.data, <uint32_t>length, DRAKEN_INT16,
    ...
    def __cinit__(self, size_t length=0, bint wrap=False):
        if wrap:
            ...
            self._unified_view = draken_vector_from_dense(NULL, 0, DRAKEN_INT16, NULL)
        else:
            self.ptr = alloc_fixed_buffer(DRAKEN_INT16, length, 2)
            self.owns_data = True
            self._unified_view = draken_vector_from_dense(
                self.ptr.data, <uint32_t>length, DRAKEN_INT16, NULL)
    ...
    def __getitem__(self, Py_ssize_t i):
        ...
        if uv.data_length == 1:
            if uv.validity != NULL:
                return None
            return <int16_t>(<int16_t*>uv.data)[0]
    ...
    def to_arrow(self):
        ...
        cdef DrakenVector* uv = self.unified()
        if uv.data_length == 1:
            val = <int16_t>(<int16_t*>uv.data)[0]
            if uv.validity != NULL:
                return pa.nulls(self.ptr.length, type=pa.int16())
            return pa.array([val] * self.ptr.length, type=pa.int16())
        cdef size_t nbytes = self.ptr.length * 2
        ...
        return pa.Array.from_buffers(pa.int16(), self.ptr.length, buffers)
    ...
    cpdef list to_pylist(self):
        ...
        cdef uint8_t byte
        cdef int16_t* d16 = <int16_t*>ptr.data
        cdef list out = []
        if uv.data_length == 1:
            for i in range(n):
                out.append(None if uv.validity != NULL else <int16_t>(<int16_t*>uv.data)[0])
            return out
        if ptr.null_bitmap == NULL:
            for i in range(n):
                out.append(d16[i])
    ...
    cpdef int64_t min(self):
        ...
        if uv.data_length == 1:
            ...
            return <int64_t>(<int16_t*>uv.data)[0]
        cdef int16_t* d16 = <int16_t*>ptr.data
        cdef int64_t m
    ...
    cpdef int64_t sum(self):
        ...
        if uv.data_length == 1:
            if uv.validity != NULL:
                return 0
            return <int64_t>(self.ptr.length * <int64_t>(<int16_t*>uv.data)[0])
        cdef DrakenFixedBuffer* ptr = self.ptr
        cdef Py_ssize_t i, n = ptr.length
    ...
    cpdef int compare_at(self, Py_ssize_t left_idx, Py_ssize_t right_idx) except? 0:
        ...
        if left_is_null or right_is_null:
            return 0
        cdef int64_t left_val = <int64_t>(<int16_t*>ptr.data)[left_idx]
        cdef int64_t right_val = <int64_t>(<int16_t*>ptr.data)[right_idx]
    ...
    cpdef Integer16Vector take(self, int32_t[::1] indices):
        cdef DrakenVector* uv = self.unified()
        cdef Py_ssize_t i, n = indices.shape[0]
        cdef bint is_const_null
        cdef Integer16Vector out
        ...
        if uv.data_length == 1:
            ...
            return Integer16Vector.from_constant(
                None if is_const_null else <int64_t>(<int16_t*>uv.data)[0],
                n,
                is_null=is_const_null,
            )
        out = Integer16Vector(<size_t>n)
        ...
        out._unified_view = draken_vector_from_dense(
            out.ptr.data, <uint32_t>n, DRAKEN_INT16, out.ptr.null_bitmap)
        return out
    ...
    cdef BoolVector _compare_scalar(self, int64_t value, int op):
        ...
        if uv.data_length == 1:
            ...
            if dispatch_compare_once(op, <int64_t>(<int16_t*>uv.data)[0], value) and nbytes > 0:
    ...
    cdef BoolVector _compare_vector(self, Integer16Vector other, int op):
        cdef DrakenVector* uv = self.unified()
        ...
        if uv.data_length == 1:
            ...
            return other._compare_scalar(<int64_t>(<int16_t*>uv.data)[0], reversed_op)
        if ouv.data_length == 1:
            ...
            return self._compare_scalar(<int64_t>(<int16_t*>ouv.data)[0], op)
    ...
    cpdef BoolVector greater_than_or_equals_vector(self, Integer16Vector other):
        return self._compare_vector(other, 3)

    cpdef BoolVector less_than(self, int64_t value):
        return self._compare_scalar(value, 4)

    cpdef BoolVector less_than_vector(self, Integer16Vector other):
    ...
    cpdef BoolVector less_than_or_equals_vector(self, Integer16Vector other):
    ...
    cdef void hash_into(
        ...
        if uv.data_length == 1:
            value = NULL_HASH if uv.validity != NULL else <uint64_t>(<int64_t>(<int16_t*>uv.data)[0])
        ...
        if offset < 0 or offset + n > out_buf.shape[0]:
            raise ValueError("Integer16Vector.hash_into: output buffer too small")
    ...
    def __str__(self):
        ...
        for i in range(k):
            vals.append(self[i])
        return f"<Integer16Vector len={buf_length(self.ptr)} values={vals}>"


cdef Integer16Vector integer16_from_arrow(object array):
    """Zero-copy wrap of a PyArrow int16/uint16 array as an Integer16Vector."""
    import pyarrow as pa

    cdef Integer16Vector vec = Integer16Vector(0, True)
    ...
    vec.ptr.type = DRAKEN_INT16
    vec.ptr.itemsize = 2
    vec.ptr.length = <size_t>len(array)
    vec.ptr.data = <void*>(base_ptr + arr_offset * 2)
    ...
    vec._unified_view = draken_vector_from_dense(
        vec.ptr.data, <uint32_t>arr_len, DRAKEN_INT16, vec.ptr.null_bitmap)
    return vec
