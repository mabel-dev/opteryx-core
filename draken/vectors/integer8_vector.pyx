...
from libc.stdint cimport int8_t, int32_t, int64_t, uint8_t, uint32_t, uint64_t, intptr_t
...
cdef class Integer8Vector(Vector):
    """Fixed-width signed 8-bit integer vector (INT8, itemsize=1)."""
    ...
    def from_constant(cls, value, length, is_null=False):
        ...
        cdef Integer8Vector vec = Integer8Vector(<size_t>length)
        ...
        vec._unified_view = draken_vector_from_constant(
            vec.ptr.data, <uint32_t>length, DRAKEN_INT8,
    ...
    def __cinit__(self, size_t length=0, bint wrap=False):
        if wrap:
            ...
            self._unified_view = draken_vector_from_dense(NULL, 0, DRAKEN_INT8, NULL)
        else:
            self.ptr = alloc_fixed_buffer(DRAKEN_INT8, length, 1)
            self.owns_data = True
            self._unified_view = draken_vector_from_dense(
                self.ptr.data, <uint32_t>length, DRAKEN_INT8, NULL)
    ...
    def __getitem__(self, Py_ssize_t i):
        ...
        if uv.data_length == 1:
            if uv.validity != NULL:
                return None
            return <int8_t>(<int8_t*>uv.data)[0]
    ...
    def to_arrow(self):
        ...
        cdef DrakenVector* uv = self.unified()
        if uv.data_length == 1:
            val = <int8_t>(<int8_t*>uv.data)[0]
            if uv.validity != NULL:
                return pa.nulls(self.ptr.length, type=pa.int8())
            return pa.array([val] * self.ptr.length, type=pa.int8())
        cdef size_t nbytes = self.ptr.length
        ...
        return pa.Array.from_buffers(pa.int8(), self.ptr.length, buffers)
    ...
    cpdef list to_pylist(self):
        ...
        cdef uint8_t byte
        cdef int8_t* d8 = <int8_t*>ptr.data
        cdef list out = []
        if uv.data_length == 1:
            for i in range(n):
                out.append(None if uv.validity != NULL else <int8_t>(<int8_t*>uv.data)[0])
            return out
        if ptr.null_bitmap == NULL:
            for i in range(n):
                out.append(d8[i])
    ...
    cpdef int64_t min(self):
        ...
        if uv.data_length == 1:
            ...
            return <int64_t>(<int8_t*>uv.data)[0]
        cdef int8_t* d8 = <int8_t*>ptr.data
        cdef int64_t m
    ...
    cpdef int64_t sum(self):
        ...
        if uv.data_length == 1:
            if uv.validity != NULL:
                return 0
            return <int64_t>(self.ptr.length * <int64_t>(<int8_t*>uv.data)[0])
        cdef DrakenFixedBuffer* ptr = self.ptr
        cdef Py_ssize_t i, n = ptr.length
    ...
    cpdef int compare_at(self, Py_ssize_t left_idx, Py_ssize_t right_idx) except? 0:
        ...
        if left_is_null or right_is_null:
            return 0
        cdef int64_t left_val = <int64_t>(<int8_t*>ptr.data)[left_idx]
        cdef int64_t right_val = <int64_t>(<int8_t*>ptr.data)[right_idx]
    ...
    cpdef Integer8Vector take(self, int32_t[::1] indices):
        cdef DrakenVector* uv = self.unified()
        cdef Py_ssize_t i, n = indices.shape[0]
        cdef bint is_const_null
        cdef Integer8Vector out
        ...
        if uv.data_length == 1:
            ...
            return Integer8Vector.from_constant(
                None if is_const_null else <int64_t>(<int8_t*>uv.data)[0],
                n,
                is_null=is_const_null,
            )
        out = Integer8Vector(<size_t>n)
        ...
        out._unified_view = draken_vector_from_dense(
            out.ptr.data, <uint32_t>n, DRAKEN_INT8, out.ptr.null_bitmap)
        return out
    ...
    cdef BoolVector _compare_scalar(self, int64_t value, int op):
        ...
        if uv.data_length == 1:
            ...
            if dispatch_compare_once(op, <int64_t>(<int8_t*>uv.data)[0], value) and nbytes > 0:
    ...
    cdef BoolVector _compare_vector(self, Integer8Vector other, int op):
        cdef DrakenVector* uv = self.unified()
        ...
        if uv.data_length == 1:
            ...
            return other._compare_scalar(<int64_t>(<int8_t*>uv.data)[0], reversed_op)
        if ouv.data_length == 1:
            ...
            return self._compare_scalar(<int64_t>(<int8_t*>ouv.data)[0], op)
    ...
    cpdef BoolVector greater_than_or_equals_vector(self, Integer8Vector other):
        return self._compare_vector(other, 3)

    cpdef BoolVector less_than(self, int64_t value):
        return self._compare_scalar(value, 4)

    cpdef BoolVector less_than_vector(self, Integer8Vector other):
    ...
    cdef void hash_into(
        ...
        if uv.data_length == 1:
            value = NULL_HASH if uv.validity != NULL else <uint64_t>(<int64_t>(<int8_t*>uv.data)[0])
        ...
        if offset < 0 or offset + n > out_buf.shape[0]:
            raise ValueError("Integer8Vector.hash_into: output buffer too small")
...
cdef Integer8Vector integer8_from_arrow(object array):
    """Zero-copy wrap of a PyArrow int8/uint8 array as an Integer8Vector."""
    import pyarrow as pa

    cdef Integer8Vector vec = Integer8Vector(0, True)
    ...
    vec.ptr.type = DRAKEN_INT8
    vec.ptr.itemsize = 1
    vec.ptr.length = <size_t>len(array)
    vec.ptr.data = <void*>(base_ptr + arr_offset)
    ...
    vec._unified_view = draken_vector_from_dense(
        vec.ptr.data, <uint32_t>arr_len, DRAKEN_INT8, vec.ptr.null_bitmap)
    return vec
