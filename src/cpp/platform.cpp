...
//
// This file intentionally exports plain C symbols so the generated Cython
// extension can import them without C++ name-mangling issues on macOS.
//
// Supported targets:
// - macOS ARM64
// - Linux x86_64
...
uint64_t get_virtual_memory_total_bytes(void) noexcept {
#if defined(__APPLE__)
    int64_t mem = 0;
...
uint64_t get_physical_memory_total_bytes(void) noexcept {
    return get_virtual_memory_total_bytes();
}

// Defined before get_free_memory_bytes so it can be called without a forward
// declaration inside the same extern "C" block.
uint64_t get_page_size_bytes(void) noexcept {
...
uint64_t get_free_memory_bytes(void) noexcept {
#if defined(__APPLE__)
    // VM_FREE_COUNT is a struct field, not a sysctl MIB constant.  Use the
    // Mach host_statistics64 API instead to obtain the free page count.
...
uint64_t get_used_memory_bytes(void) noexcept {
...
int get_cpu_count(void) noexcept {
#if defined(__APPLE__) || defined(__linux__)
...
double get_memory_utilization(void) noexcept {
    const uint64_t total = get_physical_memory_total_bytes();
    const uint64_t free = get_free_memory_bytes();
    if (total == 0 || free >= total) {
        return 0.0;
...
double get_memory_free_fraction(void) noexcept {
    const uint64_t total = get_physical_memory_total_bytes();
    const uint64_t free = get_free_memory_bytes();
    if (total == 0) {
        return 0.0;
