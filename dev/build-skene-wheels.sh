#!/bin/bash
# Build manylinux wheels for the standalone `libskene` distribution
# (import package `skene`, bundling draken — no rugo, no opteryx SQL engine).
#
# Like dev/build-rugo-wheels.sh this needs NO Rust, NO libcurl and NO OpenSSL:
# skene's extension compiles skene/src/*.cpp + vendored zstd + draken's
# extensions, and links nothing beyond libc/libm/pthread.
#
...
# same logic as dev/build-rugo-wheels.sh: a 't' request MUST resolve to a 't' ABI.
...
"${PYBIN}/python" skene/setup.py build_ext --parallel "$NPROC" bdist_wheel
...
echo "=== libskene manylinux wheels ==="
