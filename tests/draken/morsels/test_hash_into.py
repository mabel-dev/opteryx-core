...
def test_timestamp_hash_matches_arrow_vector_for_sliced_arrays():
    """TimestampVector correctly handles nulls in sliced arrays with offsets."""
    base = pa.array([None, 1, None, 2, 3, None], type=pa.timestamp("ns"))
    arr = base.slice(1, 4)  # [1, None, 2, 3]

    table = pa.table({"a": arr})
    morsel = draken.Morsel.from_arrow(table)
    native_vec = morsel.column(b"a")

    hashes = _vector_hash_to_list(native_vec)
    # position 1 is null; all others are non-null
    assert hashes[0] != 0 and hashes[0] != NULL_HASH
    assert hashes[1] == NULL_HASH
    assert hashes[2] != 0 and hashes[2] != NULL_HASH
    assert hashes[3] != 0 and hashes[3] != NULL_HASH


def test_timestamp_hash_handles_timezone_arrays_with_offsets():
    """TimestampVector correctly handles nulls when timezone arrays are sliced."""
    base = pa.array(
        [
            None,
            1,
    ...
    arr = base.slice(2, 4)  # [None, 2, 3, None]
    ...
    hashes = _vector_hash_to_list(native_vec)
    # positions 0 and 3 are null
    assert hashes[0] == NULL_HASH
    assert hashes[1] != 0 and hashes[1] != NULL_HASH
    assert hashes[2] != 0 and hashes[2] != NULL_HASH
    assert hashes[3] == NULL_HASH
...
def test_hash_into_native_int8():
    """int8 arrays are promoted to native IntegerVector."""
...
def test_hash_into_native_int16():
    """int16 arrays are promoted to native IntegerVector."""
...
def test_hash_into_native_int32():
    """int32 arrays are promoted to native IntegerVector."""
...
def test_hash_into_native_float32():
    """float32 arrays are promoted to native FloatVector."""
...
def test_hash_into_null_type_column():
    """NullVector.hash_into should handle null type arrays."""
...
def test_hash_into_int64_boundaries():
    ...
    table = pa.table(
        {"a": pa.array([9223372036854775807, -9223372036854775808, 0], type=pa.int64())}
    )
    morsel = draken.Morsel.from_arrow(table)
    vec = morsel.column(b"a")
