...
STATEMENTS = [
        ...
        ("SELECT * FROM testdata.satellites INNER HASH JOIN $planets USING (id)", None, None, SqlError),
        ...
        ("SELECT name, mission FROM testdata.astronauts INNER JOIN (SELECT * FROM UNNEST(missions) as mission) AS S ON mission = name", 0, 2, None),
        ("SELECT * FROM testdata.astronauts CROSS JOIN UNNEST(missions) AS mission WHERE mission IN ('Apollo 11', 'Apollo 12')", 6, 20, None),
        ("SELECT * FROM (SELECT ARRAY_AGG(name) as n FROM testdata.astronauts GROUP BY group) AS alma CROSS JOIN UNNEST(n) AS nn", 357, 2, None),
        ("SELECT * FROM (SELECT ARRAY_AGG(name) as n FROM testdata.astronauts WHERE status = 'Retired' GROUP BY group) AS alma CROSS JOIN UNNEST(n) AS nn", 220, 2, None),
        ("SELECT number FROM testdata.astronauts CROSS JOIN UNNEST((1, 2, 3, 4, 5)) AS number", 1785, 1, None),
        ("SELECT * FROM (SELECT ARRAY_AGG(name) as n FROM testdata.astronauts WHERE birth_date < '1960-01-01' GROUP BY group) AS alma CROSS JOIN UNNEST(n) AS nn", 269, 2, None),
        ("SELECT * FROM (SELECT ARRAY_AGG(name) as n FROM testdata.astronauts WHERE status = 'Active' AND birth_date > '1970-01-01' GROUP BY group) AS alma CROSS JOIN UNNEST(n) AS nn", 10, 2, None),
        ("SELECT group, nn FROM (SELECT group, ARRAY_AGG(name) as n FROM testdata.astronauts GROUP BY group) AS alma CROSS JOIN UNNEST(n) AS nn", 357, 2, None),
        ("SELECT * FROM (SELECT ARRAY_AGG(CASE WHEN LENGTH(alma_mater) > 10 THEN name ELSE NULL END) as alma_mater_arr FROM testdata.astronauts GROUP BY group) AS alma CROSS JOIN UNNEST(alma_mater_arr) AS alma", 357, 2, None),
        ("SELECT * FROM (SELECT ARRAY_AGG(1) as num_arr FROM testdata.astronauts GROUP BY group) AS numbers CROSS JOIN UNNEST(num_arr) AS number", 357, 2, None),
        ("SELECT name, group, astronaut_year, LEAST(year_list) FROM (SELECT ARRAY_AGG(year) as year_list, name, group FROM testdata.astronauts WHERE status = 'Retired' and year is not null GROUP BY group, name) AS alma CROSS JOIN UNNEST(year_list) AS astronaut_year", 196, 4, None),

        # PUSHDOWN (the result should be the same without pushdown)
        ("SELECT p.name, s.name FROM $planets p, testdata.satellites s WHERE p.id = s.planetId AND p.mass > 1000 AND s.gm < 500;", 63, 2, None),
        ("SELECT p.name, sub.name FROM $planets p CROSS JOIN (SELECT name, planetId FROM testdata.satellites WHERE gm < 1000) AS sub WHERE p.id = sub.planetId;", 170, 2, None),
        ("SELECT p.name, s.name FROM $planets p, testdata.satellites s WHERE p.id = s.planetId AND p.id = s.id;", 1, 2, None),
        ("SELECT p.name, COUNT(s.id) FROM $planets p JOIN testdata.satellites s ON p.id = s.planetId GROUP BY p.name HAVING COUNT(s.id) > 3;", 5, 2, None),
        ("SELECT COUNT(*) FROM $planets WHERE TRUE AND 3 = 2 AND 3 > 2", 1, 1, None),

        ("SELECT missions[0] as m FROM testdata.astronauts CROSS JOIN FAKE(1, 1) AS F order by m", 357, 1, None),
        ("SELECT name[id] as m FROM $planets", None, None, UnsupportedSyntaxError),
        ("SELECT * FROM testdata.astronauts WHERE ARRAY_CONTAINS_ANY(missions, @@user_memberships)", 3, 19, None),
        ("SELECT testdata.missions.* FROM testdata.missions INNER JOIN $user ON Mission = value WHERE attribute = 'membership'", 1, 8, None),
        ...
        ("SELECT LENGTH(name) FROM testdata.astronauts WHERE LENGTH(name) > 15", 248, 1, None),
        ("SELECT planetId, COUNT(*) FROM testdata.satellites GROUP BY planetId HAVING COUNT(*) > 10", 4, 2, None),
...
#        ("SELECT CONCAT('Astronaut: ', name) FROM testdata.astronauts WHERE LENGTH(name) > 20", 10, 1, None),
#        ("SELECT CONCAT('Hello', ' ', 'World') as concatenated", 1, 1, None),
        ...
        ("SELECT * FROM $planets LEFT JOIN (SELECT id FROM testdata.satellites WHERE planetId < 0) AS S ON $planets.id = S.id", 9, 21, None),
        ...
        ("SELECT P.name, S.name FROM $planets AS P CROSS JOIN testdata.satellites AS S WHERE P.id = S.id AND P.name LIKE '%X%'", 0, 2, None),
        # LEFT JOIN THEN FILTER ON NULLS
        ("SELECT * FROM $planets LEFT JOIN testdata.satellites ON testdata.satellites.planetId = $planets.id WHERE testdata.satellites.id IS NULL", 2, 28, None),
        ("SELECT * FROM $planets LEFT JOIN testdata.satellites ON testdata.satellites.planetId = $planets.id WHERE testdata.satellites.name IS NULL", 2, 28, None),
        # SORT BROKEN
        ("SELECT * FROM (SELECT * FROM $planets ORDER BY id DESC LIMIT 5) AS SQ WHERE id > 7", 2, 20, None),
        # ORDER OF JOIN CONDITION
        ("SELECT * FROM $planets INNER JOIN testdata.satellites ON testdata.satellites.planetId = $planets.id", 177, 28, None),
        # ORDER BY QUALIFIED IDENTIFIER
        ("SELECT * FROM $planets LEFT JOIN testdata.satellites ON testdata.satellites.planetId = $planets.id ORDER BY $planets.name", 179, 28, None),
        # NAMED SUBQUERIES
        ...
        ("SELECT COUNT(*) FROM (SELECT * FROM $planets WHERE id < 5) p LEFT JOIN testdata.satellites s ON p.id = s.planetId", 1, 1, None),
        ...
        # ("SELECT * FROM $planets WHERE id IN (SELECT planetId FROM (SELECT * FROM testdata.satellites) AS s)", 9, 20, UnsupportedSyntaxError),
        
        # Correlated subqueries - NOT YET SUPPORTED  
        # ("SELECT p.id, (SELECT COUNT(*) FROM testdata.satellites s WHERE s.planetId = p.id) AS sat_count FROM $planets p", 9, 2, UnsupportedSyntaxError),
        # ("SELECT p.name FROM $planets p WHERE EXISTS (SELECT 1 FROM testdata.satellites s WHERE s.planetId = p.id)", 9, 1, UnsupportedSyntaxError),
        # ("SELECT p.name FROM $planets p WHERE NOT EXISTS (SELECT 1 FROM testdata.satellites s WHERE s.planetId = p.id AND s.id > 1000)", 9, 1, UnsupportedSyntaxError),

        # Multiple JOIN operations - NOT YET SUPPORTED (mission table reference issue)
        # ("SELECT COUNT(*) FROM $planets p INNER JOIN testdata.satellites s ON p.id = s.planetId INNER JOIN testdata.missions m ON p.id = m.id", 9, 1, ColumnNotFoundError),
        # CORRECTED: Multiple LEFT JOINs with same table create cartesian products
        ("SELECT p.id FROM $planets p LEFT JOIN testdata.satellites s1 ON p.id = s1.planetId LEFT JOIN testdata.satellites s2 ON p.id = s2.planetId", 9167, 1, None),
        ...
        # ("SELECT id, (SELECT COUNT(*) FROM testdata.satellites WHERE planetId = $planets.id) FROM $planets", 9, 2, UnsupportedSyntaxError),
        # ("SELECT id, name, (SELECT MAX(id) FROM testdata.satellites) AS max_sat_id FROM $planets", 9, 3, UnsupportedSyntaxError),

        # Subquery in WHERE with multiple conditions - NOT YET SUPPORTED (IN subquery)
        # ("SELECT * FROM $planets WHERE id IN (SELECT planetId FROM testdata.satellites WHERE id > 10 AND id < 50)", 9, 20, UnsupportedSyntaxError),
        # ("SELECT * FROM $planets WHERE id = (SELECT MIN(planetId) FROM testdata.satellites)", 1, 20, UnsupportedSyntaxError),

        # JOIN with subqueries
        ("SELECT * FROM (SELECT id, name FROM $planets) p INNER JOIN (SELECT planetId FROM testdata.satellites) s ON p.id = s.planetId", 177, 3, None),
        # CORRECTED: Returns 1 for COUNT aggregation
        ("SELECT COUNT(*) FROM (SELECT * FROM $planets WHERE id < 5) p LEFT JOIN testdata.satellites s ON p.id = s.planetId", 1, 1, None),

        # Complex JOIN conditions with subqueries - NOT YET SUPPORTED (IN subquery)
        # ("SELECT p.id FROM $planets p WHERE id IN (SELECT planetId FROM testdata.satellites GROUP BY planetId HAVING COUNT(*) > 1)", 6, 1, UnsupportedSyntaxError),
        ...
        ("SELECT COUNT(*) FROM (SELECT id FROM $planets UNION SELECT planetId FROM testdata.satellites) AS all_ids", 1, 1, None),

        # Nested EXISTS - NOT YET SUPPORTED
        # ("SELECT * FROM $planets p WHERE EXISTS (SELECT 1 FROM testdata.satellites s WHERE s.planetId = p.id AND EXISTS (SELECT 1 FROM testdata.missions m WHERE m.id = s.id))", 9, 20, UnsupportedSyntaxError),
        ...
        ("SELECT p.id, counts.cnt FROM $planets p LEFT JOIN (SELECT planetId, COUNT(*) AS cnt FROM testdata.satellites GROUP BY planetId) counts ON p.id = counts.planetId", 9, 2, None),

        # Complex nested WITH/CTE-like subqueries - NOT YET SUPPORTED (IN subquery)
        # ("SELECT * FROM (SELECT id, name FROM $planets) AS p WHERE id IN (SELECT planetId FROM testdata.satellites WHERE id < 100)", 9, 2, UnsupportedSyntaxError),

        # Lateral-style correlations - NOT YET SUPPORTED (correlated subquery)
        # ("SELECT p.id, sub.max_sat FROM $planets p LEFT JOIN (SELECT planetId, MAX(id) AS max_sat FROM testdata.satellites WHERE planetId = p.id GROUP BY planetId) sub ON TRUE", 9, 2, UnsupportedSyntaxError),
        ...
        ("SELECT COUNT(*) FROM $planets p INNER JOIN (SELECT DISTINCT planetId FROM testdata.satellites) s ON p.id = s.planetId", 1, 1, None),
