...
from rugo.jsonl import benchmark_document_map
...
def main():
    ...
    if result:
        ...
        print(
            f"Document map building:  {result['interpret_ms']:8.1f} ms  ({interpret_throughput:7.1f} MB/s)"
        )
        print(f"{'─' * 60}")
        print(
            f"Total:                  {result['total_ms']:8.1f} ms  ({total_throughput:7.1f} MB/s)"
        )
        ...
        if result["sample_map"]:
            ...
            for i, field in enumerate(result["sample_map"][:3]):
                ...
                print(
                    f"  Field {i}: key[{key_start}:{key_end}] value[{val_start}:{val_end}] type={field['type']}"
                )
    else:
        print("FAILED")
        return 1
