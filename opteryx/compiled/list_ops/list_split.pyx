...
from libc.stdint cimport int32_t, int64_t, uint8_t, uintptr_t, uint32_t, uint64_t
...
cpdef object list_split(object arrow_array, char delimiter):
    ...
    if string_delim_counts == NULL or string_delim_starts == NULL:
        free(delim_pos)
        if string_delim_counts: free(string_delim_counts)
        if string_delim_starts: free(string_delim_starts)
    ...
    if output_data == NULL or child_offsets == NULL or list_offsets == NULL:
        ...
        if output_data: aligned_free(output_data)
        if child_offsets: aligned_free(child_offsets)
        if list_offsets: free(list_offsets)
