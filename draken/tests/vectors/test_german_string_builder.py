"""Smoke tests for GermanStringBuilder and GermanArenaHandle.
...
- Round-trip via GermanArenaHandle.to_pylist.
- Equality (gs_equals) on slot pairs within the same arena.
...
from draken.vectors.german_string_builder import GermanStringBuilder


def _build(values, *, estimate_avg_bytes=8):
    """Convenience: build a resizable arena from a list of bytes | None."""
    b = GermanStringBuilder.with_estimate(len(values), estimate_avg_bytes)
    for v in values:
        if v is None:
            b.append_null()
        else:
...
def test_equals_inline_inline():
    """gs_equals on two inline slots holding the same short bytes."""
...
def test_equals_extern_extern():
    """gs_equals on two extern slots holding the same long bytes."""
...
def test_strict_capacity_exact():
    ...
    b = GermanStringBuilder.with_counts(2, len(long_val) * 2)
    b.append(long_val)
    b.append(long_val)
    h = b.finish_handle()
...
def test_strict_capacity_underconsumed_raises():
    b = GermanStringBuilder.with_counts(1, 100)
...
def test_strict_capacity_overflow_raises():
    ...
    b = GermanStringBuilder(2, 16)  # not resizable, not strict
...
def test_incomplete_builder_raises():
    b = GermanStringBuilder.with_estimate(3, 8)
    b.append(b"only one")
...
def test_finish_called_twice_raises():
    b = GermanStringBuilder.with_estimate(1, 8)
    b.append(b"hello")
    b.finish_handle()
