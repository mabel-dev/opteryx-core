...
This asserts WHICH OPTIMIZER RULE FIRED for each statement — the second element of
each tuple is a telemetry flag, not a row count.

That distinction is why this file exists as Python rather than as a `.slt`: the
sqllogictest directives can assert a result shape, but they cannot see optimizer
telemetry. An earlier conversion to `shapes/optimizations_invoked.slt` read these
flags as row counts and emitted `query shape <n>` from them, which silently
replaced every optimizer assertion here with a result-shape assertion (several of
them wrong). The `.slt` is deleted; this is the coverage.
"""
...
STATEMENTS = [
        ...
        # Projection is qualified rather than `SELECT *`: satellites and $planets
        # share `id`/`name`, and a result carrying two columns of one name is now
        # rejected (AmbiguousIdentifierError, see ExitNode). The join ordering under
        # test is unaffected by which columns are projected.
        ("SELECT satellites.name, $planets.name FROM testdata.satellites INNER JOIN $planets ON planetId = $planets.id", "optimization_inner_join_smallest_table_left"),
...
def test_optimization_invoked(statement, flag):
    ...
    # The optimizer runs during planning, but the telemetry is only complete once
    # the plan has been executed — drain the morsels before reading it.
    session = opteryx.session(memberships=["Apollo 11", "opteryx"])
    for _ in session.execute_to_morsels(statement):
        pass
    telemetry = session.telemetry

    assert telemetry.get(flag) is not None, f"{flag} did not fire for `{statement}`"
