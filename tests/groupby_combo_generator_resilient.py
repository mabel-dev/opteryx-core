#!/usr/bin/env python
"""
GROUP BY Column Combination Test Generator - Resilient Version

Generates and executes GROUP BY tests for all combinations of columns.
Each test runs in an isolated subprocess to survive crashes/segfaults.

Usage:
    python groupby_combo_generator_resilient.py --config tests/groupby_combo_tests_config.json
    python groupby_combo_generator_resilient.py --config tests/groupby_combo_tests_config.json --limit 100
    python groupby_combo_generator_resilient.py --config tests/groupby_combo_tests_config.json --output results.json
"""
...
class ComboTestGenerator:
    ...
    def _run_test_subprocess(self, pattern_id: str, table: str, sql: str) -> TestResult:
        ...
        test_data = {"pattern_id": pattern_id, "table": table, "sql": sql}
            ...
            if result.returncode == 0:
                try:
                    ...
                    return TestResult(
                        ...
                        error_message=f"Failed to parse subprocess output: {result.stdout}",
                ...
                return TestResult(
                    ...
                    sql=sql,
                    status=TestStatus.CRASHED,
                    error_message=f"Subprocess crash: {result.stderr[:200]}",
