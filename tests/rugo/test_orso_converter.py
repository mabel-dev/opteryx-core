...
class TestOrsoConverter:
    ...
    def test_invalid_metadata(self):
        ...
        with pytest.raises(
            ValueError, match="rugo_metadata must contain 'schema_columns' or 'row_groups'"
        ):
            rugo_to_orso_schema({})
    ...
    def test_minimal_valid_metadata(self):
        ...
        minimal_metadata = {
            "num_rows": 10,
            "row_groups": [
                {"columns": [{"name": "test_col", "physical_type": "int64", "null_count": 0}]}
            ],
        }
...
def test_struct_handling():
    ...
    assert any(
        col.name == "configurations" and col.type == OrsoTypes.JSONB for col in orso_schema.columns
    )
    ...
    assert any(
        col.name == "publishedDate" and col.type == OrsoTypes.VARCHAR for col in orso_schema.columns
    )
    assert any(
        col.name == "lastModifiedDate" and col.type == OrsoTypes.VARCHAR
