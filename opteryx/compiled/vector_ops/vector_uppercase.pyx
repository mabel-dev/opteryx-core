...
cpdef StringVector vector_uppercase(object input):
    ...
    cdef DrakenGermanArena* uc_gdv
    cdef GermanString* uc_slot
    cdef const uint8_t* uc_sdata
    cdef uint32_t uc_slen
    ...
    if vec._german_dict_values != NULL:  # dictionary
        uc_gdv = vec._german_dict_values
        dict_size = <Py_ssize_t>uc_gdv.length

        dict_builder = string_vector_module.StringVectorBuilder.with_estimate(dict_size, 16)
        for i in range(dict_size):
            uc_slot = &uc_gdv.slots[i]
            uc_slen = gs_length(uc_slot)
            uc_sdata = gs_data(uc_slot, uc_gdv.arena)
            tmp_dict_buf = <char*>malloc(<size_t>uc_slen)
            if tmp_dict_buf == NULL:
                raise MemoryError()
            try:
                memcpy(tmp_dict_buf, uc_sdata, <size_t>uc_slen)
                simd_to_upper(tmp_dict_buf, <size_t>uc_slen)
                dict_result_bytes = PyBytes_FromStringAndSize(tmp_dict_buf, <Py_ssize_t>uc_slen)
