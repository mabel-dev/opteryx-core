...
"""
German-string arena helpers — allocate / free DrakenStringArena.
...
`alloc_string_arena(dtype, length, arena_cap)` allocates the header,
DrakenStringSlot slot array, and (when arena_cap > 0) the byte arena.
...
`free_string_arena(a)` releases owned buffers and the header. Slot data
...
cdef inline DrakenStringArena* alloc_string_arena(DrakenType dtype, size_t length, size_t arena_cap):
    cdef DrakenStringArena* a = <DrakenStringArena*> malloc(sizeof(DrakenStringArena))
    if a == NULL:
        raise MemoryError()

    if length > 0:
        a.slots = <DrakenStringSlot*> malloc(length * sizeof(DrakenStringSlot))
        if a.slots == NULL:
            free(a)
            raise MemoryError()
    else:
...
cdef inline void free_string_arena(DrakenStringArena* a) noexcept:
    if a == NULL:
        return
    if a.owns_buffers != 0:
        if a.slots != NULL:
