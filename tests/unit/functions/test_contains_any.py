"""
Tests for the Draken-native list_contains_any.

New signature:
    list_contains_any(ArrayVector, set) -> BoolVector
"""
...
class TestListContainsAnyBasic:
    def test_single_row_match(self):
        ...
        assert _result(list_contains_any(vec, {1})) == [True]

    def test_single_row_no_match(self):
        vec = _array_vec([[1, 2, 3]])
        assert _result(list_contains_any(vec, {9})) == [False]

    def test_multiple_rows_mixed(self):
        vec = _array_vec([[1, 2], [3, 4], [5, 6]])
        assert _result(list_contains_any(vec, {1, 5})) == [True, False, True]

    def test_all_rows_match(self):
        vec = _array_vec([[10], [10, 20], [10, 20, 30]])
        assert _result(list_contains_any(vec, {10})) == [True, True, True]

    def test_no_rows_match(self):
        vec = _array_vec([[1, 2], [3, 4]])
        assert _result(list_contains_any(vec, {99, 100})) == [False, False]

    def test_stops_at_first_match(self):
        vec = _array_vec([[5, 1, 2, 3]])
        assert _result(list_contains_any(vec, {5})) == [True]
...
class TestListContainsAnyEmpty:
    def test_empty_vector(self):
        vec = _array_vec([])
        assert _result(list_contains_any(vec, {1})) == []

    def test_empty_items_set(self):
        vec = _array_vec([[1, 2], [3, 4]])
        assert _result(list_contains_any(vec, set())) == [False, False]

    def test_empty_row_in_array(self):
        vec = _array_vec([[], [1, 2]])
        assert _result(list_contains_any(vec, {1})) == [False, True]

    def test_all_empty_rows(self):
        vec = _array_vec([[], [], []])
        assert _result(list_contains_any(vec, {1})) == [False, False, False]
...
class TestListContainsAnyNulls:
    ...
    def test_all_null_rows(self):
        ...
        assert _result(list_contains_any(vec, {1})) == [False, False]

    def test_null_among_matches(self):
        vec = _array_vec([[3, 1], None, [9]])
        assert _result(list_contains_any(vec, {1, 9})) == [True, False, True]
...
class TestListContainsAnyStrings:
    def test_string_match(self):
        ...
        assert _result(list_contains_any(vec, {b"banana", b"cherry"})) == [True, True]

    def test_string_no_match(self):
        vec = _str_array_vec([["foo", "bar"]])
        assert _result(list_contains_any(vec, {b"baz"})) == [False]

    def test_mixed_rows(self):
        vec = _str_array_vec([["a", "b", "c"], ["d", "e"], ["f"]])
        assert _result(list_contains_any(vec, {b"a", b"f"})) == [True, False, True]
