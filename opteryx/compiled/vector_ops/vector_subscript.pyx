...
# Integer-indexed subscript kernels.
#
# These are non-JSON operations that perform element/character access by
# integer index.  JSON object field access (by string key) lives in the
# nanobind vector_json module (Milestone E.18).
#
# Moved from vector_map_access.pyx (deleted in E.18).
...
cpdef StringVector vector_map_access_string(StringVector vec, Integer64Vector key):
    """
    Character subscript over StringVector using a constant Integer64Vector key.
