...
# Always access arr._child, cast to StringVector, then use unified().


cdef inline bytes _child_bytes(DrakenStringArena* arena, Py_ssize_t j):
    """Extract bytes for child element j from a DrakenStringArena."""
    cdef uint32_t slen = str_length(&arena.slots[j])
    cdef const uint8_t* sptr = str_data(&arena.slots[j], arena.arena)
    return PyBytes_FromStringAndSize(<char*>sptr, <Py_ssize_t>slen)
...
cpdef StringVector vector_concat_array(ArrayVector arr):
    ...
    # child lives in _child.
    cdef StringVector child_sv = <StringVector>arr._child
    cdef DrakenVector* child_uv = child_sv.unified()
    cdef DrakenStringArena* child_arena = <DrakenStringArena*>child_uv.data
    cdef Py_ssize_t n = arr_ptr.length
    cdef uint8_t* row_null_bm = arr_ptr.null_bitmap
    cdef uint8_t* child_null_bm = child_uv.validity
    cdef Py_ssize_t i
    ...
    for i in range(n):
        ...
        for j in range(child_start, child_end):
            ...
            if child_null_bm != NULL and not ((child_null_bm[j >> 3] >> (j & 7)) & 1):
                continue
            parts.append(_child_bytes(child_arena, j))
