WITH ssr AS
  (SELECT s_store_id AS store_id,
   ...
   LEFT OUTER JOIN testdata.tpcds_tiny.store_returns ON (ss_item_sk = sr_item_sk
                                     AND ss_ticket_number = sr_ticket_number), testdata.tpcds_tiny.date_dim,
                                                                               testdata.tpcds_tiny.store,
                                                                               testdata.tpcds_tiny.item,
                                                                               testdata.tpcds_tiny.promotion
  ...
  (SELECT cp_catalog_page_id AS catalog_page_id,
   ...
   LEFT OUTER JOIN testdata.tpcds_tiny.catalog_returns ON (cs_item_sk = cr_item_sk
                                       AND cs_order_number = cr_order_number), testdata.tpcds_tiny.date_dim,
                                                                               testdata.tpcds_tiny.catalog_page,
                                                                               testdata.tpcds_tiny.item,
                                                                               testdata.tpcds_tiny.promotion
  ...
  (SELECT web_site_id,
   ...
   LEFT OUTER JOIN testdata.tpcds_tiny.web_returns ON (ws_item_sk = wr_item_sk
                                   AND ws_order_number = wr_order_number), testdata.tpcds_tiny.date_dim,
