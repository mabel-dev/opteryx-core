...
// enclosing DrakenStringArena's `arena` buffer — position-independent so slot
// arrays can be memcpy'd / IPC'd / persisted without pointer fixup. Helpers
// that read the payload (str_data, str_equals, str_compare) therefore take an
...
// Builder-side initializers. DrakenStringSlot is opaque to Cython; field writes go
...
static inline void str_init_inline(DrakenStringSlot* s, const uint8_t* src, uint32_t length) {
    // Precondition: length <= STR_INLINE_MAX. Caller chose the inline form.
    memset(s, 0, sizeof(DrakenStringSlot));
    s->inl.length = length;
    if (length > 0) {
...
static inline void str_init_extern(DrakenStringSlot* s, const uint8_t* src,
    ...
    // Precondition: length > STR_INLINE_MAX (so src has >= 13 bytes — the
...
static inline int str_compare(const DrakenStringSlot* a, const uint8_t* arena_a,
    ...
    const uint8_t* pa = str_data(a, arena_a) + 4;
    const uint8_t* pb = str_data(b, arena_b) + 4;
    c = memcmp(pa, pb, mn - 4);
    if (c != 0) return c;
