...
cdef int _resolve_profile(str where, str compression, str profile) except -1:
    """Map a profile name to its enum, rejecting contradictions.

    The zstd level is policy, not a caller knob: it is chosen per column from
    the column's physical type (see zstd_level_for in _parquet_writer.hpp).
    "fast" is the default and is what CTAS and uploads want; "storage" raises
    only the BYTE_ARRAY level, for the defragmenter's rewrite of bytes that are
    then read many times. Asking for a profile with compression="none" is a
    contradiction and is rejected rather than silently ignored.
    """
    if profile == "fast":
        return PROFILE_FAST
    if profile == "storage":
        if compression != "zstd":
            raise ValueError(
                "%s: profile='storage' requires compression='zstd', got %r"
                % (where, compression))
        return PROFILE_STORAGE
    raise ValueError(
        "%s: profile must be 'fast' or 'storage', got %r" % (where, profile))
...
def write_parquet(Morsel morsel not None, str compression="zstd",
    ...
    profile: "fast" (default) or "storage" — selects the compression effort.
        The zstd level is never passed in; it is chosen per column from the
        column's physical type. "fast" is for CTAS/upload latency; "storage"
        raises only the BYTE_ARRAY level, for the defragmenter. Requires
        compression="zstd"; pairing it with "none" raises ValueError.
        Independently of the profile, a column chunk whose compressed form is
        not smaller than its raw form is stored raw and marked UNCOMPRESSED.
    ...
    sorted_by: name of a column whose values the CALLER asserts are already
        ...
        each row group's parquet sorting_columns footer field — rugo does
        NOT verify it. A downstream reader only trusts this claim when the
        file's created_by identifies rugo as the writer (see metadata.cpp);
        an untrue hint is a correctness bug in the caller, not something
    ...
    sorted_descending: sort direction for `sorted_by` (default ascending).
        ...
        (matches draken's own null-ordering convention).
...
cdef _encode(Morsel morsel, str compression, bint want_bounds, object bloom_filters,
    ...
    # Resolve the sort-order hint's column name once (caller-asserted, not
    # verified — see write_parquet's docstring).
    cdef str sorted_by_name = None
    if sorted_by is not None:
        sorted_by_name = sorted_by.decode("utf-8") if isinstance(sorted_by, bytes) else sorted_by
    if compression == "zstd":
        codec = CODEC_ZSTD
    ...
    # depth-2 nested ARRAY (list<list<scalar>>) locals
    cdef Vector leafv
    cdef const DrakenVector* mid_dv
    cdef const DrakenVector* leaf_dv
    cdef const int32_t* offs_mid
    cdef const uint32_t* mid_sel
    cdef const uint8_t* mid_val
    cdef const uint32_t* leaf_sel
    cdef const uint8_t* leaf_val
    cdef DrakenType leaf_t
    cdef int32_t mstart, mend, lstart, lend
    cdef uint32_t mp, lp
    ...
    for i in range(ncols):
        ...
        if sorted_by_name is not None:
            nm = names[i]
            nm = nm.decode("utf-8") if isinstance(nm, bytes) else nm
            if nm == sorted_by_name:
                ci.sorted_hint = True
                ci.sorted_descending = sorted_descending
                ci.sorted_nulls_first = not sorted_descending
...
cdef class _StreamingParquetWriter:
    ...
    max_page_bytes mirror write_parquet and apply to every row group.
    """
    cdef StreamingParquetWriter* _w
    cdef object _sink
    cdef str _compression
    cdef str _profile
    ...
    def __cinit__(self, object sink, str compression="zstd", object bloom_filters=True,
        ...
        if compression == "zstd":
            ...
            raise ValueError(
                "open_parquet_writer: compression must be 'zstd' or 'none', got %r"
                % (compression,))
        self._sink = sink
    ...
    cdef void _drain(self) except *:
        cdef vector[uint8_t] pending
        with nogil:
            pending = self._w.take_pending()
        if pending.size() > 0:
            self._sink(PyBytes_FromStringAndSize(
    ...
    def write_row_group(self, Morsel morsel not None):
        ...
        if morsel._num_columns() == 0:
            raise ValueError("write_row_group: morsel has no columns")
    ...
    def __exit__(self, exc_type, exc, tb):
        # Only finalise (write the footer) on a clean exit; on an exception the
        # partial file is abandoned rather than capped with a footer over a
