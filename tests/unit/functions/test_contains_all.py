"""
Tests for the Draken-native list_contains_all.

New signature:
    list_contains_all(ArrayVector, set) -> BoolVector
"""
...
class TestListContainsAllBasic:
    def test_single_item_present(self):
        ...
        assert _result(list_contains_all(vec, {1})) == [True]

    def test_single_item_absent(self):
        vec = _array_vec([[1, 2, 3]])
        assert _result(list_contains_all(vec, {9})) == [False]

    def test_all_items_present(self):
        vec = _array_vec([[1, 2, 3]])
        assert _result(list_contains_all(vec, {1, 2, 3})) == [True]

    def test_some_items_missing(self):
        vec = _array_vec([[1, 2, 3]])
        assert _result(list_contains_all(vec, {1, 4})) == [False]

    def test_multiple_rows(self):
        vec = _array_vec([[1, 2, 3], [1, 3], [1, 2, 3, 4]])
        assert _result(list_contains_all(vec, {1, 2, 3})) == [True, False, True]

    def test_items_superset_of_row(self):
        vec = _array_vec([[1, 2]])
        assert _result(list_contains_all(vec, {1, 2, 3})) == [False]

    def test_items_subset_of_row(self):
        vec = _array_vec([[1, 2, 3, 4, 5]])
        assert _result(list_contains_all(vec, {2, 4})) == [True]
...
class TestListContainsAllEmpty:
    def test_empty_vector(self):
        vec = _array_vec([])
        assert _result(list_contains_all(vec, {1})) == []

    def test_empty_items_trivially_true(self):
        vec = _array_vec([[1, 2], [3, 4], []])
        assert _result(list_contains_all(vec, set())) == [True, True, True]

    def test_empty_row_nonempty_items(self):
        vec = _array_vec([[], [1, 2]])
        assert _result(list_contains_all(vec, {1})) == [False, True]

    def test_all_empty_rows(self):
        vec = _array_vec([[], [], []])
        assert _result(list_contains_all(vec, {1})) == [False, False, False]
...
class TestListContainsAllNulls:
    ...
    def test_all_null_rows(self):
        ...
        assert _result(list_contains_all(vec, {1})) == [False, False]

    def test_null_among_matches(self):
        vec = _array_vec([[1, 2], None, [1, 2, 3]])
        assert _result(list_contains_all(vec, {1, 2})) == [True, False, True]

    def test_null_row_with_empty_items(self):
        # Empty items: non-null rows are True, null rows are False
        vec = _array_vec([None, [1, 2]])
        assert _result(list_contains_all(vec, set())) == [False, True]
...
class TestListContainsAllSemantics:
    ...
    def test_extra_row_elements_ok(self):
        vec = _array_vec([[1, 2, 3, 4, 5]])
        assert _result(list_contains_all(vec, {1, 3})) == [True]

    def test_duplicate_row_elements(self):
        vec = _array_vec([[1, 1, 2, 2]])
        assert _result(list_contains_all(vec, {1, 2})) == [True]
...
class TestListContainsAllStrings:
    def test_all_present(self):
        ...
        assert _result(list_contains_all(vec, {b"apple", b"cherry"})) == [True]

    def test_missing_one(self):
        vec = _str_array_vec([["apple", "banana"]])
        assert _result(list_contains_all(vec, {b"apple", b"cherry"})) == [False]

    def test_mixed_rows(self):
        vec = _str_array_vec([["a", "b", "c"], ["a", "c"], ["b"]])
        assert _result(list_contains_all(vec, {b"a", b"c"})) == [True, True, False]
