...
class DrakenInnerJoinNode(JoinNode):
    ...
    def _project_morsel(self, morsel: Morsel, keep_names: list[str]) -> Morsel:
        encoded_keep = [name.encode("utf8") for name in keep_names]
                ...
                if morsel == EOS:
                    ...
                    if self.columns is not None and self.left_morsel.num_rows > 0:
                        ...
                        left_keep = [
                            ...
                            if name
                            in {
                                col.decode("utf8") if isinstance(col, bytes) else col
                                for col in self.left_morsel.column_names
                            }
                        ]
                        for join_col in self.left_columns:
                            if isinstance(join_col, bytes):
                                join_name = join_col.decode("utf8")
                            else:
                                join_name = str(join_col)
                            if join_name not in left_keep:
                                left_keep.append(join_name)
                        if left_keep:
                ...
                for chunk in self._iter_morsels(draken):
                    ...
                    if self.columns is not None:
                        ...
                        right_keep = [
                            ...
                            in {
                                col.decode("utf8") if isinstance(col, bytes) else col
                                for col in right_chunk.column_names
                            }
                        ]
                        for join_col in self.right_columns:
                            if isinstance(join_col, bytes):
                                ...
                                join_name = str(join_col)
                            if join_name not in right_keep:
                                right_keep.append(join_name)
                        if right_keep:
