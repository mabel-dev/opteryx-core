...
def test_is_numeric_vector_type_recognizes_numeric_arrays():
    assert is_numeric_vector_type(OrsoTypes.ARRAY, OrsoTypes.DOUBLE)
    assert is_numeric_vector_type(OrsoTypes.ARRAY, OrsoTypes.INTEGER)
    assert is_numeric_vector_type(OrsoTypes.ARRAY, OrsoTypes.DECIMAL)


def test_is_numeric_vector_type_rejects_non_numeric_arrays():
...
def test_node_is_numeric_vector_prefers_schema_column_types():
    node = SimpleNamespace(
        type=None,
        element_type=None,
        schema_column=SimpleNamespace(type=OrsoTypes.ARRAY, element_type=OrsoTypes.DOUBLE),
    )
