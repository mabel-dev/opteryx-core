#pragma once
//
// Templated comparison kernels for Float32Vector.
//
...
// match the existing null-density gating in float32_vector.pyx:
...
namespace draken { namespace float32_cmp {
...
struct Eq { static inline bool apply(float a, float b) { return a == b; } };
struct Ne { static inline bool apply(float a, float b) { return a != b; } };
struct Gt { static inline bool apply(float a, float b) { return a >  b; } };
struct Ge { static inline bool apply(float a, float b) { return a >= b; } };
struct Lt { static inline bool apply(float a, float b) { return a <  b; } };
struct Le { static inline bool apply(float a, float b) { return a <= b; } };
...
// byte granularity. No per-row branching on either side.
template <typename Op>
...
// One side has a null bitmap, the other does not. Byte-batched.
template <typename Op>
...
// Both sides have null bitmaps. Branchless variant. Byte-batched.
template <typename Op>
...
// per-row branching. op codes match float32_vector.pyx convention:
//   0=eq  1=ne  2=gt  3=ge  4=lt  5=le
// ---------------------------------------------------------------------------

static inline bool dispatch_compare_once(int op, float a, float b) {
    switch (op) {
