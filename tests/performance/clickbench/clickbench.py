...
STATEMENTS = [
        ...
        ("/* 39 */ SELECT URL, COUNT(*) AS PageViews FROM {DATASET} WHERE CounterID = 62 AND EventDate::TIMESTAMP[ms] >= '2013-07-01'::TIMESTAMP[ms] AND EventDate::TIMESTAMP[ms] <= '2013-07-31'::TIMESTAMP[ms] AND IsRefresh = 0 AND IsLink <> 0 AND IsDownload = 0 GROUP BY URL ORDER BY PageViews DESC LIMIT 10 OFFSET 1000;", None),
        ("/* 40 */ SELECT TraficSourceID, SearchEngineID, AdvEngineID, CASE WHEN (SearchEngineID = 0 AND AdvEngineID = 0) THEN Referer ELSE '' END AS Src, URL AS Dst, COUNT(*) AS PageViews FROM {DATASET} WHERE CounterID = 62 AND EventDate::TIMESTAMP[ms] >= '2013-07-01'::TIMESTAMP[ms] AND EventDate::TIMESTAMP[ms] <= '2013-07-31'::TIMESTAMP[ms] AND IsRefresh = 0 GROUP BY TraficSourceID, SearchEngineID, AdvEngineID, CASE WHEN (SearchEngineID = 0 AND AdvEngineID = 0) THEN Referer ELSE '' END, URL ORDER BY PageViews DESC LIMIT 10 OFFSET 1000;", None),
        ("/* 41 */ SELECT URLHash, EventDate, COUNT(*) AS PageViews FROM {DATASET} WHERE CounterID = 62 AND EventDate::TIMESTAMP[ms] >= '2013-07-01'::TIMESTAMP[ms] AND EventDate::TIMESTAMP[ms] <= '2013-07-31'::TIMESTAMP[ms] AND IsRefresh = 0 AND TraficSourceID IN (-1, 6) AND RefererHash = 3594120000172545465 GROUP BY URLHash, EventDate ORDER BY PageViews DESC LIMIT 10 OFFSET 100;", None),
        ("/* 42 */ SELECT WindowClientWidth, WindowClientHeight, COUNT(*) AS PageViews FROM {DATASET} WHERE CounterID = 62 AND EventDate::TIMESTAMP[ms] >= '2013-07-01'::TIMESTAMP[ms] AND EventDate::TIMESTAMP[ms] <= '2013-07-31'::TIMESTAMP[ms] AND IsRefresh = 0 AND DontCountHits = 0 AND URLHash = 2868770270353813622 GROUP BY WindowClientWidth, WindowClientHeight ORDER BY PageViews DESC LIMIT 10 OFFSET 10000;", None),
