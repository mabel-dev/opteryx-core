...
cdef class GermanStringBuilder:
    """Builder for DrakenGermanArena (German-string values payload).

    Mirrors the API shape of StringVectorBuilder but produces the new format:
    a fixed-width GermanString slot array (16 bytes each) plus a byte arena
    for payloads > GS_INLINE_MAX (12) bytes.
    ...
    """
    cdef DrakenGermanArena* _arena    # owned until finish() detaches
    ...
    cdef GermanString* _slots          # cached pointer into _arena.slots
    ...
    # longer references it; the caller must free_german_arena() it eventually
...
cdef class GermanArenaHandle:
    """Test/inspection wrapper around a finished DrakenGermanArena.

    Owns the arena: dealloc calls free_german_arena. Not used by production
    ...
    """
    cdef DrakenGermanArena* _arena
    cdef bint _owns

    @staticmethod
    cdef GermanArenaHandle _wrap(DrakenGermanArena* arena)

    cdef GermanString* _slot(self, Py_ssize_t i) except NULL
    ...
    # Pairwise equality check (smoke test for gs_equals on this arena).
