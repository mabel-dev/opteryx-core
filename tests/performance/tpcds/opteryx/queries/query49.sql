     ...
     (SELECT item,
      ...
      FROM
        (SELECT ws.ws_item_sk AS item,
         ...
         FROM testdata.tpcds_tiny.web_sales ws
         LEFT OUTER JOIN testdata.tpcds_tiny.web_returns wr ON (ws.ws_order_number = wr.wr_order_number
                                            AND ws.ws_item_sk = wr.wr_item_sk) ,testdata.tpcds_tiny.date_dim
     ...
     (SELECT item,
      ...
      FROM
        (SELECT cs.cs_item_sk AS item,
         ...
         FROM testdata.tpcds_tiny.catalog_sales cs
         LEFT OUTER JOIN testdata.tpcds_tiny.catalog_returns cr ON (cs.cs_order_number = cr.cr_order_number
                                                AND cs.cs_item_sk = cr.cr_item_sk) ,testdata.tpcds_tiny.date_dim
     ...
     (SELECT item,
      ...
      FROM
        (SELECT sts.ss_item_sk AS item,
         ...
         FROM testdata.tpcds_tiny.store_sales sts
         LEFT OUTER JOIN testdata.tpcds_tiny.store_returns sr ON (sts.ss_ticket_number = sr.sr_ticket_number
                                              AND sts.ss_item_sk = sr.sr_item_sk) ,testdata.tpcds_tiny.date_dim
