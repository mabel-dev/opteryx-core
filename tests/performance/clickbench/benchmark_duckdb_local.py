...
Output: duckdb.local.json  (same schema as duckdb.c6a.4xlarge.json)

Usage:
    python tests/performance/clickbench/benchmark_duckdb_local.py [--parquet-dir scratch/hits]

The result file is written to the same directory as this script so that
clickbench.py can reference it as the local baseline.
"""
...
def main():
    ...
    output_path = args.output or os.path.join(os.path.dirname(__file__), "duckdb.local.json")
