...
def extract_pyarrow(path: str):
    ...
    md = pf.metadata
    out = {"rows": md.num_rows, "row_groups": md.num_row_groups, "columns": []}
...
def compare(pa, cu):
    ...
    for i, (pa_col, cu_rg) in enumerate(zip(pa["columns"], cu["row_groups"][0]["columns"])):
        ...
        if pa_col["physical_type"] != cu_rg["physical_type"]:
            diffs.append(
                f"Col {i} type mismatch: {pa_col['physical_type']} vs {cu_rg['physical_type']}"
            )
        if pa_col.get("nulls") != cu_rg.get("null_count"):
            diffs.append(
                f"Col {i} nulls mismatch: {pa_col.get('nulls')} vs {cu_rg.get('null_count')}"
            )

        # Skip min/max comparison for fixed_len_byte_array with decimal logical types
        # We don't decode decimal types yet
        if cu_rg["physical_type"] == "fixed_len_byte_array" and cu_rg.get(
            "logical_type", ""
        ).startswith("decimal"):
            continue
        ...
        if pa_col.get("min") != cu_rg.get("min"):
            diffs.append(
                f"Col {i} min mismatch: `{pa_col.get('min')}` vs `{cu_rg.get('min')}` ({cu_rg['physical_type']})"
            )
        if pa_col.get("max") != cu_rg.get("max"):
            diffs.append(
                f"Col {i} max mismatch: `{pa_col.get('max')}` vs `{cu_rg.get('max')}` ({cu_rg['physical_type']})"
            )
    return diffs
...
def run_one(file: str, iters=100) -> bool:
    ...
    print(
        f"[Custom] rows={cu['num_rows']} groups={len(cu['row_groups'])} time={custom_time:.3f} ms"
    )
    if diffs:
        print("❌ Differences:")
        for d in diffs:
