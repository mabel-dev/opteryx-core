        ...
        for rg_idx, rg in enumerate(meta["row_groups"]):
            ...
            for col in rg["columns"]:
                if "." not in col["name"]:
                    ...
                    print(
                        f"    {col['name']:20} | physical={col['physical_type']:12} | logical={logical or '(none)'}"
                    )
    ...
    for file_path in files_to_test:
        ...
        for col in meta["row_groups"][0]["columns"]:
            if "." not in col["name"]:
                logical = col.get("logical_type", "")
                print(
                    f"    {col['name']:25} | physical={col['physical_type']:17} | logical={logical or '(none)':<17}  | arrow={arrow_types.get(col['name'], '(missing)')}"
                )
