...
#        ("SELECT * FROM $planets AS P1 UNION SELECT * FROM $planets AS P2;", 9, 20, UnsupportedSyntaxError),
        ...
        ("SELECT P0.id, P1.ID FROM $planets AS P0 LEFT JOIN (SELECT id, name AS N FROM $planets) AS P1 ON P0.name = P1.N", 9, 2, None),
...
#        ("SELECT surface_pressure * 1 + surface_pressure * 0 AS opt FROM $planets", 4, 1, None),
        ...
        # Can't mod floats
        ("SELECT mass % 1, mass FROM $planets", 9, 2, None),
]
# fmt:on
