...
cdef class Integer8Vector(Vector):
    ...
    def __getitem__(self, Py_ssize_t i):
        ...
        if uv.validity != NULL and not ((uv.validity[i >> 3] >> (i & 7)) & 1):
            return None
        return <int8_t>(<int8_t*>uv.data)[uv.selection[i]]
    ...
    cpdef list to_pylist(self):
        ...
        cdef int8_t* data = <int8_t*>uv.data
        cdef list out = []
        for i in range(n):
            if uv.validity != NULL and not ((uv.validity[i >> 3] >> (i & 7)) & 1):
                out.append(None)
    ...
    cpdef Integer8Vector take(self, int32_t[::1] indices):
        ...
        cdef Integer8Vector out = Integer8Vector(<size_t>n)
        cdef int8_t* data = <int8_t*>uv.data
        cdef int8_t* dst = <int8_t*>out.ptr.data
    ...
    cdef BoolVector _compare_vector(self, Integer8Vector other, int op):
        ...
        cdef Py_ssize_t n = <Py_ssize_t>uv.length
        cdef int8_t* data1 = <int8_t*>uv.data
        cdef int8_t* data2 = <int8_t*>ouv.data
        cdef Py_ssize_t i
        cdef Py_ssize_t nbytes = (n + 7) >> 3
        cdef BoolVector out
        cdef uint8_t* dst
