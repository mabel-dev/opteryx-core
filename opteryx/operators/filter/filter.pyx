...
WHERE execution is 100% native (src/cpp/engine/engine.hpp's ExprFilterOperator —
see opteryx/managers/execution/compiler.py's FilterNode branch, which reads
`.filter`/`._const_replacements` off this class and compiles them straight into
the engine). This class is plan-time config only — the native compiler never
calls a push/execute method on it, and none exists here.
"""
