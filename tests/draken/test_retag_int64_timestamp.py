...
from draken.draken_native import vector_retag_int64_as_timestamp64

UTC = datetime.timezone.utc


def test_retag_seconds_keeps_value_verbatim():
    v = vector_int64_from_sequence([1370000000, 75])
...
def test_retag_units_distinct():
    ...
    assert vector_retag_int64_as_timestamp64(vector_int64_from_sequence([1000000]), "s").to_pylist() == [
        datetime.datetime(1970, 1, 12, 13, 46, 40, tzinfo=UTC)
    ]
    assert vector_retag_int64_as_timestamp64(vector_int64_from_sequence([1000000]), "us").to_pylist() == [
        datetime.datetime(1970, 1, 1, 0, 0, 1, tzinfo=UTC)
    ]


def test_source_is_emptied_after_move():
    v = vector_int64_from_sequence([1, 2, 3])
...
def test_non_int64_rejected():
    with pytest.raises(Exception):
        vector_retag_int64_as_timestamp64(vector_int64_from_sequence([1.5, 2.5]), "s")
