...
static int command_info(const char* path) {
    ...
    if (meta.block_row_groups > 0)
        std::printf("block     %u row groups\n", meta.block_row_groups);
    if (!meta.writer_tag.empty())
        std::printf("writer    %s\n", meta.writer_tag.c_str());
    std::printf("\n");

    // The row group table, from the footer alone. This IS the pruning view a
    // ranged reader gets from one read.
    for (size_t i = 0; i < meta.row_groups.size(); ++i) {
        ...
        std::printf("row group %zu  rows=%" PRIu64 " first_row=%" PRIu64 "\n",
                    i, rg.row_count, rg.first_row);
    }
    std::printf("\n");

    // Per-column detail for row group 0 only: on a packed file dumping every
    // row group buries the thing an operator opened the file to see.
