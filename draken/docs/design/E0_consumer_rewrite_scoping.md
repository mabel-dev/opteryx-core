...
> E.18: Phase 17 cost datapoint added (JSON cluster: vector_json_extract + vector_map_access via yyjson). Pure C′ + Part A (draken/ops/json_extract.h). 2 .pyx deleted (vector_json_extract, vector_map_access); non-JSON subscript ops moved to new vector_subscript.pyx. yyjson compiled as C11 source in the Extension block (not pre-built .o). 19/19 direct validation tests passing.
> E.19: Phase 18 cost datapoint added (lighter special remainders: subscript + math). 2 .pyx deleted (vector_subscript, vector_math). Determinations: vector_get_element — dead code (zero callers, confirmed by grep; split in a prior milestone per setup.py comment); deleted without replacement. vector_map_access_string + vector_map_access_array → new vector_special.cpp (NB_MODULE). vector_ops.pyx — auto-generated by setup.py consolidator; cannot be cleaned up until all constituents migrated (11 .pyx remain). vector_math.pyx (6 ops: ceil/floor/trunc/power/random/random_normal) → extended vector_math.cpp (E.3) + extended float_math.h (new fm_ceil/floor/trunc_scaled helpers + fm_ceil/floor/trunc/power_tmpl templates + float_ceil/floor/trunc/power dispatch). RNG: std::mt19937_64 (uniform: random_device seed; normal: fixed seed 674162347314 for reproducibility). Callers updated: arithmetic.pyx (6 imports), binary_operators.pyx (1 import). 48 new tests, 2792/2792 draken native tests green.
...
#### Phase 1 (E.18) — C′ + Part A: JSON extraction cluster (vector_json_extract + vector_map_access) ✅ DONE

**Cost datapoint (actual, 2026-05-23):**

| Item | LOC | Notes |
|---|---|---|
| Part A: `draken/ops/json_extract.h` — JDocGuard RAII + dotpath_to_jsonptr | ~60 | Header-only; dot-notation → RFC 6901 JSON Pointer; handles `$.a.b[0]`, `/a/b`, bare `name` |
| Part B: `opteryx/compiled/nanobind/vector_json.cpp` — one NB_MODULE, 2 functions | ~300 | Shared `impl_extract` core; growing arena via `std::vector<uint8_t>` then `draken_malloc` copy; `std::free` on `yyjson_val_write` output (libc malloc); RAII slots guard |
| `opteryx/compiled/vector_ops/vector_subscript.pyx` — non-JSON subscript ops | ~120 | Relocated from deleted `vector_map_access.pyx`; auto-picked up by `generate_consolidated_module()` |
| `draken/tests/native/test_vector_json.py` — 33 tests | ~295 | dot-notation, JSON Pointer, array indices, null TVL, composable chaining, extern slot, TypeError, RuntimeError |
| `opteryx/expression/binary_operators.pyx` — ArrowOp + LongArrowOp updated | 4 | Import now from `opteryx.compiled.nanobind.vector_json` |
| `setup.py` — new E.18 Extension block | ~18 | yyjson.c compiled as C11 source (not pre-built .o) to avoid parse-time `os.path.exists` race |
| Deleted: `vector_json_extract.pyx`, `vector_map_access.pyx` | ~350 removed | |

**Surprises / flags for subsequent consumers:**

- **yyjson linking via source, not pre-compiled .o**: `extra_objects=["build/temp.yyjson.o"] if os.path.exists(...)` is evaluated at setup.py parse time (before `build_extensions()` pre-compiles the `.o`). On first build the `.o` didn't exist yet → linker saw undefined `_unsafe_yyjson_ptr_getx`. Fix: include `"third_party/yyjson/src/yyjson.c"` directly as a source; setuptools compiles `.c` with the C compiler (C11), not the C++ compiler — correct behaviour.
- **`std::free` vs `draken_free` for `yyjson_val_write` output**: `yyjson_val_write` allocates with `libc malloc`. Must use `std::free` (not `draken_free` / mimalloc) to release it.
- **`emit_null` slot index**: output slots are indexed by logical row `i`, not the dict `code`. Any future consumer emitting null into a dense output slot array must use `i`, not `selection[i]`.
- **Non-JSON subscript ops in `vector_map_access.pyx`**: ticket spec deleted the file, but it contained `vector_get_element`, `vector_map_access_array`, `vector_map_access_string` (integer-indexed subscript, not JSON). Created `vector_subscript.pyx` to host them; `MapAccessOp` in `binary_operators.pyx` unchanged.
- **`vector_from_string_sequence` accepts `str` or `None` only** — passing `bytes` raises `ValueError`. Test helpers must decode bytes to `str` before constructing a test vector.
- **`Vector.__getitem__` returns `str`, not `bytes`** — test assertions must use plain `"..."` string literals, not `b"..."`.
