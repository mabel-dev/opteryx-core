...
STATEMENTS = [
        ...
        ("Select * From testdata.satellites", 177, 8, None),
        ("SELECT   *   FROM   testdata.satellites", 177, 8, None),
        ("SELECT\n\t*\n  FROM\n\ttestdata.satellites", 177, 8, None),
        ("\n\n\n\tSELECT * FROM testdata.satellites", 177, 8, None),
        ("SELECT * FROM testdata.satellites\t;", 177, 8, None),
        ("SELECT *\tFROM testdata.satellites", 177, 8, None),
        ("  SELECT * FROM testdata.satellites", 177, 8, None),
        ...
        ("SELECT * \n/* comment */\n FROM testdata.satellites", 177, 8, None),
        ("SELECT * FROM testdata.satellites\n-- comment\n;", 177, 8, None),
        ("/* comment */SELECT * FROM testdata.satellites--comment", 177, 8, None),
        ("SELECT * FROM testdata.satellites;--comment", 177, 8, None),
        ("SELECT * --comment\nFROM testdata.satellites", 177, 8, None),
...
-- AND name = 'Earth')
        ...
        ("SELECT testdata.satellites.* FROM testdata.satellites", 177, 8, None),
        ("SELECT s.* FROM testdata.satellites AS s", 177, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name = 'Calypso'", 1, 8, None),
        ("SELECT * FROM testdata.satellites WHERE (name = 'Calypso')", 1, 8, None),
        ("SELECT * FROM testdata.satellites WHERE NOT name = 'Calypso'", 176, 8, None),
        ("SELECT * FROM testdata.satellites WHERE NOT (name = 'Calypso')", 176, 8, None),
        ("SELECT * FROM testdata.satellites WHERE `name` = 'Calypso'", 1, 8, None),
        ("select * from testdata.satellites where name = 'Calypso'", 1, 8, None),
        ("select * from testdata.satellites where name == 'Calypso'", 1, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name <> 'Calypso'", 176, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name < 'Calypso'", 21, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name <= 'Calypso'", 22, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name > 'Calypso'", 155, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name >= 'Calypso'", 156, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name is null", 0, 8, None),
        ("SELECT * FROM testdata.satellites WHERE not name is null", 177, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name is not null", 177, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name is true", 0, 8, IncorrectTypeError),
        ("SELECT * FROM testdata.satellites WHERE not name is true", 177, 8, IncorrectTypeError),
        ("SELECT * FROM testdata.satellites WHERE name is not true", 177, 8, IncorrectTypeError),
        ("SELECT * FROM testdata.satellites WHERE name is false", 0, 8, IncorrectTypeError),
        ("SELECT * FROM testdata.satellites WHERE not name is false", 177, 8, IncorrectTypeError),
        ("SELECT * FROM testdata.satellites WHERE name is not false", 177, 8, IncorrectTypeError),
        ("SELECT * FROM testdata.satellites WHERE name != 'Calypso'", 176, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name = '********'", 0, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name LIKE '_a_y_s_'", 1, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name LIKE 'Cal%%'", 4, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name LIKE 'Cal%'", 4, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name like 'Cal%'", 4, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name ILIKE '_a_y_s_'", 1, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name ILIKE 'Cal%'", 4, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name ilike 'Cal%'", 4, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name RLIKE '.a.y.s.'", 1, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name RLIKE '^Cal.*'", 4, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name rlike '^Cal.*'", 4, 8, None),
        ("SELECT * FROM testdata.satellites WHERE name RLIKE '(?i)cal.*'", 4, 8, None),
        ("SELECT * FROM testdata.satellites WHERE TRUE", 177, 8, UnsupportedSyntaxError),
        ("SELECT * FROM testdata.satellites WHERE FALSE", 0, 8, UnsupportedSyntaxError),
        ("SELECT * FROM testdata.satellites WHERE NOT TRUE", 0, 8, UnsupportedSyntaxError),
        ("SELECT * FROM testdata.satellites WHERE NOT FALSE", 177, 8, UnsupportedSyntaxError),
        ("SELECT * FROM testdata.satellites WHERE `name` = 'Calypso'", 1, 8, None),
        ("SELECT * FROM `testdata.satellites` WHERE name = 'Calypso'", 1, 8, None),
        ("SELECT * FROM `testdata.satellites` WHERE `name` = 'Calypso'", 1, 8, None),
        ("SELECT * FROM testdata.satellites WITH (NO_CACHE)", 177, 8, None),
